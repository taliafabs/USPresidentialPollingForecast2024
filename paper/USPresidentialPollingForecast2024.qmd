...
subtitle: "Vice President Kamala Harris Projected to Win Exactly 270 Electoral Votes and 48.1% of the Popular Vote Based on Poll of Polls and Bayesian Modeling Analysis"
author: 
  - Talia Fabregas
  - Fatimah Yunusa
  - Aliza Mithwani
...
Based on the results of applying our Bayesian model, we estimate that Vice President Harris will receive x% of the popular vote and win the swing states of ... and former President Trump will receive x% of the popular vote and win the swing states of ...
...
We cleaned this dataset to only include high-quality polls at the national and state-level conducted on or after July 21, 2024. We determined which polls were high-quality based on the numeric grade, and cleaned the dataset to include only polls with a numeric grade of at least 3.0. Polls with a rating (`numeric_grade`) of 3.0 are conducted by the best pollsters in the United States and score in the 99th percentile or better for accuracy and transparency [@MorrisBestPollsters].
...
ggplot() +
  ...
  labs(
    x = "Days since Biden withdrew",
    y = "",
    title = "Poll Percentage over Time with Spline Fit (Kamala Harris vs Donald Trump) Arizona"
  ) +
  theme_minimal() +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  scale_color_manual(values = c("Kamala Harris" = "blue", "Donald Trump" = "red")) +
